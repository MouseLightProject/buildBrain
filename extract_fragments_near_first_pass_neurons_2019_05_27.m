...
output_folder_path = sprintf('/groups/mousebrainmicro/mousebrainmicro/cluster/Reconstructions/%s/build-brain-output', sample_date) ;
all_fragments_file_path = fullfile(output_folder_path, 'frags.mat') ;
...
tic_id = tic() ;
fragments_as_swc_arrays_in_erhan_coords = load_anonymous(all_fragments_file_path) ;
...
first_pass_neurons_and_names = compute_or_read_from_memo(output_folder_path, ...
...
swc_folder_path = fullfile(output_folder_path, 'frags-near-first-pass-neurons') ;
save_swc_arrays(swc_folder_path, ...                         
                nearby_fragments) ;
