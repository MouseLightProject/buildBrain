function [outtree,selectedIdx] = sampleTree_old(intree,opt,params)
...
% Copyright: HHMI 2016
[L,list] = getBranches_old(intree.dA);
...
for ii=1:length(L)
    ...
    if 1
        ...
        xyz = XYZ(set_ii,:);
        xyz = xyz.*(ones(size(xyz,1),1)*params.voxres);
        switch opt.sampling_style
            case 'uni'
        ...
        if cdist(end)>opt.sampling_interval
            % make sure tip is in the list
            [aa,idx]=min(pdist2([0:opt.sampling_interval:cdist(end)-opt.sampling_interval]',cdist(:)),[],2);
