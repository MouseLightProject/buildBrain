...
% Copyright: HHMI 2016
if nargin<1
    configfile = './config_files/config_buildBrain_20170925_prob0_stratum.cfg';
...
gt_swcfolder = '/nrs/mouselight/seggui/swcfiles/GT'
if ~isempty(maskids)
    %%
    [hits_allen_brain] = maskWithAllenAtlas(params,subs,maskids);
    if exist('gt_swcfolder','var')
        addpath(genpath('./scripts'))
        [hits_gt,hits_delete] = cropSectionBasedOnGT(params,gt_swcfolder,subs);
        % keep allen_brain and gt then substract delete
        keepthese = setdiff(union(hits_allen_brain,hits_gt),hits_delete);
